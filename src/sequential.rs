...
impl Sequential {
    ...
    pub fn new() -> Sequential {
        Sequential { layers: Vec::new::<>(), weights: vec![] }
    }
    ...
    pub fn fit(&mut self, x_train: &Tensor, y_train: &Tensor, epochs: u32, verbose: bool) {
        ...
        for iteration in 0..epochs {
            ...
            for i in 0..x_train.shape[0] {
                ...
                gradients.push(gradient.mult_el(&(self.weights.last().unwrap() * &outputs[outputs.len() - 2]).map(&self.layers.last().unwrap().activation.deriv_activation())));
                ...
                for (i, w) in self.weights.iter().skip(1).rev().enumerate() {
                    ...
                    let right_gradient = (&self.weights[self.weights.len() - 2 - i] * &outputs[outputs.len() - 3 -i]).map(&self.layers[self.layers.len() - 2 - i].activation.deriv_activation());
